            ...
            {filteredMoreItems.length > 0 && (
                <div className="pt-2">
                    <button
                        ...
                        className={`group w-full flex items-center justify-between px-2 py-2 font-medium rounded-md text-gray-600 hover:bg-gray-50 hover:text-gray-900 dark:text-gray-300 dark:hover:bg-gray-700 dark:hover:text-white ${isMobile ? 'text-base' : 'text-sm'}`}
                        aria-expanded={isMoreOpen}
                    >
                        <div className="flex items-center">
                            <MoreIcon className="mr-3 flex-shrink-0 h-6 w-6" />
                            <span>More Options</span>
                        </div>
                        <ChevronDownIcon
                            className={`h-5 w-5 transform transition-transform duration-150 ${isMoreOpen ? 'rotate-180' : ''}`}
                        />
                    </button>
                    {isMoreOpen && (
                        <div className="mt-1 space-y-1 pl-5 animate-fadeIn" style={{ animationDuration: '0.3s' }}>
